...
def prompt1(datapoint): # 该prompt中添加了info
    ...
    if datapoint['assemgraph_com']['nodenum'] == 1:
        ...
        prompt4input = Template("""Within $add_info, please understand the following assembly code and data mapping table (defines the correspondence between data labels and their actual values), and perform decompilation into corresponding high-level C source code.
        ...
        prompt4input = Template("""Within $add_info, please understand the following control flow graph and data mapping table (defines the correspondence between data labels and their actual values), and perform decompilation into corresponding high-level C source code.
...
def prompt2(datapoint): # 该prompt中没有添加info
    datapoint["data"] = json.loads(datapoint["data"])
    datapoint["assemgraph_com"] = json.loads(datapoint["assemgraph_com"])
...
def build_instruct(datapoint):
    assembly_prompt = {
        ...
        "bit width": datapoint['mode'],
        "compiler optimization level": datapoint['opts'],
        "assembly code": datapoint['comassem']}
    cfg_prompt = {
		"a list of cfg_blocks": datapoint['assemgraph']['nodes'],
		"edges between two connected cfg_blocks": datapoint['assemgraph']['edges'], # (1,3)表示nodes对应的列表中，第1个节点和第3个节点之间存在关联边，节点的索引从0开始
		"cfg_block count": datapoint['assemgraph']['nodenum'] # 该函数体所能抽取出来的block的数量
	    }
    assembly_prompt = json.dumps(assembly_prompt)
    cfg_prompt = json.dumps(cfg_prompt)

    # prompt4input = f"You are a professional decompilation assistant. Your task is to analyze the input assembly code and control flow graph (CFG) information, understand the function's logic, and derive the corresponding high-level source code. Please ensure that the output source code is well-structured, syntactically correct, and accurately reflects the logic and functionality of the assembly code.\n- The assembly code is obtained by disassembling the binary file (compiled by GCC) using objdump, and its content is represented as a JSON dictionary: \n{assembly_prompt}\n\n- The control flow graph (CFG) information is extracted by IDA from the binary file and is represented as another JSON dictionary: \n{cfg_prompt}\n\nEach 'cfg_block' in 'a list of cfg_blocks' contains: \n  - 'assemblock': The assembly instructions within the block.\n  - 'bytecode': The hexadecimal bytecode corresponding to the block. \n  - 'addr': The tuple of start and end addresses of the instructions.\n\nThe generated target source code is: "
    prompt4input = f"The assembly code is obtained by disassembling the binary file (compiled by GCC) using objdump, and its content is represented as a JSON dictionary: \n{assembly_prompt}\n\n- The control flow graph (CFG) information is extracted by IDA from the binary file and is represented as another JSON dictionary: \n{cfg_prompt}\n\nEach 'cfg_block' in 'a list of cfg_blocks' contains: \n  - 'assemblock': The assembly instructions within the block.\n  - 'bytecode': The hexadecimal bytecode corresponding to the block. \n  - 'addr': The tuple of start and end addresses of the instructions.\n\nThe high-level source code is: "

    return prompt4input


def supervised_tokenize_pretrain(
    data_point: Dict[str, Any], tokenizer: LlamaTokenizer, ignore_index: int = None, max_length: int = 4096
# ) -> Dict[str, Union[int, str, List[int]]]:
    ...
    # source_text = prompt1(data_point)  # prompt中添加了info
    source_text = prompt2(data_point)  # prompt中没有添加info
    target_text = data_point["sourcecode"]  # `str`
    ...
    num_negative_100 = sequence_labels.count(IGNORE_INDEX)
    if num_negative_100 < 2500: # 保留足够的目标标签，长度为4096-2500
        return dict(
    ...
    return {"input_ids": [], "labels": [], "seq_length": 0}
